...
function getRecommendation(answers: Answers): PackageId {
  ...
  if (role === 'business' || goal === 'support' || goal === 'automation') return 'support-bot';
  if (role === 'researcher' || goal === 'research') return 'research';
  return 'copywriter';
}

const packageDetails: Record<PackageId, { name: string; price: string; bestFor: string; description: string; includes: string[]; url: string }> = {
  copywriter: {
    ...
    includes: ['Recommended model on OpenRouter', 'Prompt templates for copywriting', 'OpenClaw skill suggestions', 'Setup guide PDF'],
    url: 'https://clawmatic.gumroad.com/l/jymqu',
  },
  'support-bot': {
    ...
    includes: ['Recommended model for Q&A + support', 'Example conversation flows', 'OpenClaw setup guide', '30-min onboarding call'],
    url: 'https://clawmatic.gumroad.com/l/imsche',
  },
  research: {
    ...
    includes: ['Recommended model for deep reasoning', 'Research workflow guide', 'Prompt templates for analysis', 'OpenClaw skill suggestions'],
    url: 'https://clawmatic.gumroad.com/l/pcdgmu',
  },
};
      ...
      <div className="min-h-screen bg-background flex items-center justify-center pt-16">
        <div className="container mx-auto px-4 sm:px-6 max-w-2xl py-16">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">{pkg.name}</h1>
            <p className="text-muted-foreground">Based on your answers, this is the best fit for you.</p>
          </div>
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-8 mb-6">
            <div className="flex items-start justify-between gap-4 mb-4">
              <p className="text-xs text-muted-foreground"><strong className="text-foreground/70">Best for:</strong> {pkg.bestFor}</p>
            ...
            <Button variant="hero" asChild className="w-full">
              <a href={pkg.url} target="_blank" rel="noopener noreferrer">
                Get this stack <ArrowRight className="ml-1 h-4 w-4" />
              </a>
            </Button>
          </div>
          <p className="text-center text-sm text-muted-foreground">
            Want to compare?{' '}
            <a href="/packages" className="text-primary hover:underline">See all packages →</a>
          </p>
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center pt-16">
      <div className="container mx-auto px-4 sm:px-6 max-w-xl py-16">
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{currentStep.question}</h1>
        </div>
        ...
        <Button variant="hero" onClick={handleNext} disabled={!selected} className="w-full">
