...
export default function GuidesPage() {
  ...
  const filtered = activeTag === 'All' ? guides : guides.filter((g) => g.tags.includes(activeTag));
    ...
    <div className="min-h-screen bg-background">
      <section className="pt-32 pb-14 relative border-b border-border/50">
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight mb-4">OpenClaw AI Guides</h1>
            ...
            {allTags.map((tag) => (
              <button
                ...
                className={`px-4 py-3.5 text-xs font-medium whitespace-nowrap border-b-2 transition-colors ${
                  tag === activeTag ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'
                }`}
              >
                {tag}
              </button>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
            {filtered.map((guide) => (
              <article key={guide.slug} className="group rounded-xl border border-border/50 bg-card p-6 flex flex-col card-hover">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed mb-5 flex-1">{guide.description}</p>
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                  <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock className="h-3.5 w-3.5" />
                    {guide.readTime} read
                  </span>
                  <Link href={`/guides/${guide.slug}`} className="flex items-center gap-1 text-xs font-medium text-primary hover:underline">
